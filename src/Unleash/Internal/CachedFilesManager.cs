    ...
    internal class FileSystemBackupManager : BackupManager
    {
        ...
        public void Save(Backup backup)
        {
            try
            {
                ...
                WriteBackup(settings.GetFeatureToggleFilePath(), backup.FeatureState);
                WriteBackup(settings.GetFeatureToggleETagFilePath(), backup.ETag);
            }
            catch (Exception ex)
            {
        ...
        private Backup LoadMainBackup()
        {
            try
            {
                string toggleFileContent = settings.FileSystem.ReadAllText(settings.GetFeatureToggleFilePath());
                string etagFileContent = settings.FileSystem.ReadAllText(settings.GetFeatureToggleETagFilePath());
        ...
        private Backup LoadLegacyBackup()
        {
            try
            {
                string toggleFileContent = settings.FileSystem.ReadAllText(settings.GetLegacyFeatureToggleFilePath());
                string etagFileContent = settings.FileSystem.ReadAllText(settings.GetLegacyFeatureToggleETagFilePath());
